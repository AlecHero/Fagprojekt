...
def main():
    ...
    centroids = []
    for k in range(K):
        centroids.append(np.loadtxt('synthetic_data/synthetic/centroids/synth_cov_p'+str(p)+'K'+str(K)+'_'+str(k+1)+'.csv',delimiter=','))

    # load synthetic dataset generated from the MACG model
    data = np.loadtxt('synthetic_data/synthetic/synth_data_MACG_p'+str(p)+'K'+str(K)+'_1.csv',delimiter=',')
    n = data.shape[0]
